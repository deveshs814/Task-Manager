...
const AddTask = ({ setAddTaskDiv }) => {
  ...
  const addTask = async(e) =>{
    e.preventDefault();
    try {
      const res = await axios.post("http://localhost:8000/api/v1/addTask",
        Values,
  ...
  return (
    <div className="bg-white rounded px-4 py-4 w-[40%]">
      ...
      <form className="flex flex-col gap-4">
        ...
        ></textarea>
        <div className="flex items-center justify-between gap-4">
          <button className="w-full bg-blue-800 hover:bg-blue-700 text-white transition-all duration-300  rounded" onClick={addTask}>
            Add Task
          </button>
          <button
